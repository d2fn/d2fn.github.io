...
bq. *Axiom #1: A service isn’t "fully designed" until it’s in production, has acceptable stability, and is is fast enough for your needs.*
...
h3. Why should you write a functional specification?

The primary goal of a functional specification is to prevent a significant percentage of conceptual and design errors from entering the implementation phase of a program or, worse, becoming part of a fully designed program (per axiom #1). Catching these problems early means we can address them more cheaply. During the process, you and your colleagues should also begin to form a clear mental model of the work before it is fully designed.


h3. What should be covered in a functional specification?
...
If the service exposes a public interface, document its behavior before building it. You’ll need to revisit once the system is fully designed (per axiom #1) but, since this is externally visible behavior, the importance of addressing during specification can't be overstated. If you are describing an HTTP REST API, give attention to each endpoint and its inputs and outputs.

*Specify messaging and interactions with other services*
Services rarely run in isolation. With rare exception, integration points exist with other services. A functional specification should describe these integration points. See axiom #2 for why this part of a specification is so important. Be as specific as you can. This means documenting both the _syntax_ and the _semantics_ of any messaging between services in addition to the intended transport. Describe what happens in the event of message loss, for example. If you plan to be reliant on HTTP idempotency, say so.

Specifying these interactions can be tricky because the boundary between _what_ the service does and _how_ it does it blurs. This is normal. Don't fret. Just be aware of this fact and choose your words appropriately. It is usually helpful to frame this discussion in terms of the blackbox work done earlier.
...
Mindful attention to failure pathology is a critical part of the specification. Bear in mind that failures tend to cascade across dependent services. A quick review of 
distributed computing fallacies":http://en.wikipedia.org/wiki/Fallacies_of_Distributed_Computing is often helpful.


h3. How do you know when to write a functional specification?
...
In short, a functional specification should be written for any significant software undertaking. In some cases, this isn’t so easy. For example, sometimes we confuse the pervasive design goals of a whole product as something that should be broken into discrete engineering tasks. If you find yourself referring to disparate systems when talking about it, it’s probably a meta-concern and can’t be fully reasoned about with an acutely targeted functional spec. If this is the case, it's a good idea to write down the goals in specific terms and get an understanding of the points of impact on the rest of the system. From there, you will have a better understanding of where to focus your energy when it comes time to specify any work that falls out. This may need to be repeated a number of times before it's clear what actually needs to be built.


h3. How to know when to stop writing the spec and start implementing the spec
...
Finally, it is tempting to skip many of these suggestions if you are working in a small team or trying to ship something quickly. It is likely that adaptations are a good idea in many cases as there is no one right way to plan an engineering project. But follow through on functional specifications is a good way to slow the buildup of technical debt. This is a major problem which tends to plague teams that are moving fast such as a startup in their first year. It may seem counter-intuitive, but functional specifications can actually help you go faster.
