...
The first chart shows how the animal kingdom as a whole sees the greatest growth in diversity of life and overall species counts. But when considering the phylum we see that those in the <a href="http://en.wikipedia.org/wiki/Monera">Monera</a>, <a href="http://en.wikipedia.org/wiki/Fungus">Fungi</a>, and <a href="http://en.wikipedia.org/wiki/Algae">Algae</a> kingdom's dominate the top slots--with the exception of an appearance from <a href="http://en.wikipedia.org/wiki/Phoronid">Phoronida</a> (also known as horseshoe worms).
...
fr.parse("/data/fr.csv", function(list) {
    ...
    habitatTimetable(totalGraphContext, totalByHabitat, totalTimeline);
