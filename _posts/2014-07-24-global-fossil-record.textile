...
First of all, at least in terms of number of distinct species we know about, the majority of life on Earth appears to be marine-based. However, this listing makes clear trends towards terrestrial life forms. So far our data agrees with generally held scientific believes on the origin of life so we are off to a good start.

With this idea in min
...
fr.parse("/data/fr.csv", function(list) {
    ...
    treerings(600, d3.select("svg#tree-rings"), fr.periodDetails(), fr.habitats);
...
function chart(selection, speciesList, timegrid) {
    ...
    var svg = selection.attr({width: width, height: height});
    ...
    var dividers = d3.range(columns).filter(function(n) { return n > 0; });
    divG.selectAll("line")
        .data(dividers)
        .enter()
        .append("line")
        ...
        .attr("x1", function(n) {
            return columnXScale(n) - (columnXScale(0)*(4/columns));
        })
        .attr("y1", function(n) {
